	...
	void* Allocate(size_t Bytes)
	{
		...
		while (Current)
		{
			if (Current->IsFree && Current->Size >= Bytes)
			{
				...
				{
					MemoryBlock* NewBlock = reinterpret_cast<MemoryBlock*>(reinterpret_cast<Byte>(Current) + sizeof(MemoryBlock) + Bytes);
				...
				return reinterpret_cast<Byte>(Current) + sizeof(MemoryBlock);
			}
